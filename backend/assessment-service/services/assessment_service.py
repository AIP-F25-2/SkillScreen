...
class AssessmentService:
    ...
    async def generate_assessment(self, interview_id: str, custom_weights: Optional[Dict[str, float]] = None) -> Dict:
        ...
        with UnitOfWork() as uow:
            ...
            if not completion_status['all_complete']:
                # Check if we have stuck sessions (timeout)
                if completion_status['stuck_sessions']:
                    # We have stuck sessions - generate partial assessment
                    logger.warning(
                        "generating_partial_assessment_timeout",
                        interview_id=interview_id,
                        stuck_sessions=completion_status['stuck_sessions']
                    )
                    # Continue with partial assessment generation
            ...
            if custom_weights:
                ...
                # LLM weights disabled via config
                ...
                # Get job description for LLM weight determination
                ...
                if job_position_id:
                    ...
                    if job_position:
                        ...
                        # Check if weights are actually from LLM or defaults
            ...
            if is_partial_assessment:
                # Get session info to identify which questions are stuck
            ...
            proctoring_events = self._collect_proctoring_events(interview_id)
            ...
            assessment_data = {
                ...
                'recommendation': llm_result['recommendation'].lower().replace(' ', '_'),  # Convert to enum format
                'evidence_clips': {
                    'video_clips': evidence_clips,
                    'proctoring_events': proctoring_events,
                    'service_results': service_raw_results,  # Full JSON from each service
                    'scoring_weights': weights,  # Show what weights were used
                    'weights_source': weights_source,  # 'llm_determined', 'custom', or 'default'
    ...
    def _aggregate_audio_scores(self, audio_analyses: List[Dict]) -> Dict:
        ...
        for analysis in audio_analyses:
            ...
            if comm_data.get('communication_score'):
                communication_scores.append(float(comm_data['communication_score']) * 10)  # Convert 0-10 to 0-100
            
            # Confidence score
            conf_data = raw_results.get('confidence_analysis', {})
            if conf_data.get('confidence_score'):
                confidence_scores.append(float(conf_data['confidence_score']) * 10)  # Convert 0-10 to 0-100
    ...
    def _aggregate_text_scores(self, text_analyses: List[Dict]) -> Dict:
        ...
        for analysis in text_analyses:
            ...
            if tech_assessment.get('score'):
                technical_scores.append(float(tech_assessment['score']) * 10)  # Convert 0-10 to 0-100
            
            comm_assessment = summary.get('communication_assessment', {})
            if comm_assessment.get('score'):
                communication_scores.append(float(comm_assessment['score']) * 10)  # Convert 0-10 to 0-100
    ...
    def _calculate_proctoring_risk_from_video_audio(
        ...
        for analysis in audio_analyses:
            ...
            if cheating_detection.get('cheating_detected', False):
                risk_scores.append(80.0)  # High risk
        ...
        if risk_scores:
            ...
            return min(100.0, avg_risk)  # Cap at 100
        
        return 10.0  # Default low risk if no data
    
    def _calculate_proctoring_risk_from_events(self, proctoring_events: List[Dict]) -> float:
        """
        Calculate risk score from proctoring events (alternative method)
        ...
        for event in proctoring_events:
            ...
            risk_points += severity_weights.get(severity, 20)
    ...
    def _extract_audio_highlights(self, audio_analyses: List[Dict]) -> Dict:
        ...
        # Use first analysis as representative (or aggregate if needed)
    ...
    def _collect_proctoring_events(self, interview_id: str) -> List[Dict]:
        """
